...
class TransformerForecaster(BaseForecaster):
    def __init__(self) -> None:
        super().__init__(name="transformer_exog", is_global=True)
        self._model = None
    ...
    def fit(self, train_df: pd.DataFrame, config: Dict) -> None:
        ...
        if len(X_train) == 0:
            raise ValueError("Not enough data to build Transformer sequences (check lookback / feature NaNs).")
        ...
        class TransformerRegressor(nn.Module):
            def __init__(self, input_size: int, d_model: int = 64, nhead: int = 4, num_layers: int = 2, dropout: float = 0.1, max_len: int = 52):
                super().__init__()
        ...
        self._device = device
        model = TransformerRegressor(input_size=X_train.shape[-1], max_len=self._lookback).to(device)
